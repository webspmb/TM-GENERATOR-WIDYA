...
export default function ModulTable({ data, formInput, onBack }: ModulTableProps) {
  ...
  // Fungsi pembantu untuk mengubah text sensitif menjadi gambar SVG (tidak bisa diedit teksnya di Word)
  const convertTextToProtectedImage = (text: string, fontSize: number = 14, isBold: boolean = false) => {
    const cleanText = text || "DOKUMEN ASLI";
    const width = cleanText.length * (fontSize * 0.65);
    const height = fontSize + 10;
    const fontWeight = isBold ? "bold" : "normal";
    
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}">
      <text x="0" y="${fontSize}" font-family="'Times New Roman', serif" font-size="${fontSize}px" font-weight="${fontWeight}" fill="#000000">${cleanText}</text>
    </svg>`;
    
    const base64Svg = btoa(unescape(encodeURIComponent(svg)));
    
    // Proteksi tambahan menggunakan style bawaan MS Word mso-element agar gambar menyatu dengan teks latar dan sulit diseleksi/dihapus
    return `<img src="data:image/svg+xml;base64,${base64Svg}" alt="${cleanText}" style="vertical-align: middle; display: inline-block; pointer-events: none; -webkit-user-select: none; user-select: none; mso-element: field-end;" />`;
  };

  const downloadWord = () => {
    if (!containerRef.current) return;
    
    const schoolNameText = data.identitas.schoolName || formInput.schoolName || "DOKUMEN ASLI"; 
    
    // Jalankan kloning container dokumen asli agar manipulasi ekspor tidak merusak layar web pengguna
    const cloneContainer = containerRef.current.cloneNode(true) as HTMLDivElement;
    
    // 1. Amankan nama sekolah pada tabel identitas kloning
    const schoolCell = cloneContainer.querySelector('.target-school-name');
    if (schoolCell) {
      schoolCell.innerHTML = convertTextToProtectedImage(schoolNameText, 14, true);
    }

    // 2. Amankan nama Kepala Sekolah dan nama Guru di area Tanda Tangan dokumen kloning
    const principalCell = cloneContainer.querySelector('.target-principal-signature');
    if (principalCell) {
      principalCell.innerHTML = convertTextToProtectedImage(formInput.principalName, 14, true);
    }

    const teacherCell = cloneContainer.querySelector('.target-teacher-signature');
    if (teacherCell) {
      teacherCell.innerHTML = convertTextToProtectedImage(formInput.teacherName, 14, true);
    }
        ...
        <div class="Section1">
          ...
          <div style='mso-element:footer' id='f1'>
            <p class="MsoFooter" style="border-top: 1pt solid black; padding-top: 5pt; color: #666666;">
              ${schoolNameText} — TM Generator App
            ...
            <table className="spreadsheet-table">
              <tbody>
                <tr>
                  ...
                  <td className="target-school-name">{data.identitas.schoolName}</td>
                </tr>
                <tr>
            ...
            <tbody>
              <tr>
                <td className="w-1/2 text-left align-top p-0 border-none">
                  ...
                  <div className="mt-20"> 
                    {/* Menggunakan penampung kelas target-principal-signature */}
                    <p className="font-bold mb-0 target-principal-signature" dangerouslySetInnerHTML={{ __html: convertTextToProtectedImage(formInput.principalName, 14, true) }} />
                ...
                <td className="w-1/2 text-left align-top p-0 border-none">
                  ...
                  <div className="mt-20">
                    {/* Menggunakan penampung kelas target-teacher-signature */}
                    <p className="font-bold mb-0 target-teacher-signature" dangerouslySetInnerHTML={{ __html: convertTextToProtectedImage(formInput.teacherName, 14, true) }} />
